...
parser.add_argument("-c", dest="cov_mat_raw", help="Output raw coverage matrix as sparse csr gzip numpy object (*.npz)",action='store', default='none')
parser.add_argument("-n", dest="cov_mat_norm", help="Output double normalized coverage matrix as sparse csr gzip numpy object (*.npz)",action='store', default='none')
args = parser.parse_args()

#%%
'''Functions'''

def main(path_to_p_file, path_to_sample_names_file, path_to_outgroup_boolean_file, path_to_list_of_quals_files, path_to_list_of_diversity_files, path_to_candidate_mutation_table, path_to_cov_mat_raw, path_to_cov_mat_norm, flag_cov_raw, flag_cov_norm ):
    ...
    for i in range(numSamples):
        ...
        with gzip.open(paths_to_diversity_files[i],'rb') as f:
            data = pickle.load(f)
        counts[:,:,i]=data[p-1,0:8].T #-1 convert position to index
    ...
    if flag_cov_raw:
        ...
        with gzip.open(path_to_cov_mat_raw, 'wb') as f: 
            pickle.dump(all_coverage_per_bp, f,protocol=4) # protocol=4 for storage of files >4gb
    if flag_cov_norm:
        print("Saving "+path_to_cov_mat_norm)
        with gzip.open(path_to_cov_mat_norm, 'wb') as f: 
            pickle.dump(array_cov_norm_scaled, f,protocol=4) # protocol=4 for storage of files >4gb
    ...
    with gzip.open(path_to_candidate_mutation_table, 'wb') as f: 
        ...
        pickle.dump(CMT, f,protocol=4) # protocol=4 for storage of files >4gb
...
if __name__ == "__main__":
    ...
    if path_to_cov_mat_norm == 'none':
        ...
        flag_cov_norm=True
    main(path_to_p_file, path_to_sample_names_file, path_to_outgroup_boolean_file, path_to_list_of_quals_files, path_to_list_of_diversity_files, path_to_candidate_mutation_table, path_to_cov_mat_raw, path_to_cov_mat_norm, flag_cov_raw, flag_cov_norm )
